...
export default function ResumeAnalyzerPage() {
  ...
  const { data: resumes = [], isLoading } = useQuery<ResumeListItem[]>({
    ...
    refetchInterval: (query) => {
      ...
      if (
        data &&
        data.some((r) => r.status === "PROCESSING" || r.status === "UPLOADED")
      ) {
        return 3000;
      }
      return false;
        ...
        {resumes.length > 0 && (
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <StatusSummaryCard
              icon={
                <Loader2
                  className={`h-4 w-4 ${counts.processing > 0 ? "animate-spin" : ""}`}
                />
              }
              label="Processing"
...
function StatusSummaryCard({
  ...
  return (
    <div
      className={`flex items-center gap-3 border rounded-lg px-4 py-3 ${toneStyles[tone]} ${bgStyles[tone]}`}
    >
