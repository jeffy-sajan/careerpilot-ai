...
async def create(session: AsyncSession, user_id: uuid.UUID, token_hash: str, expires_at: datetime) -> RefreshToken:
...
async def revoke(session: AsyncSession, token_id: uuid.UUID) -> None:
    ...
    await session.execute(delete(RefreshToken).where(RefreshToken.id == token_id))
    await session.commit()
