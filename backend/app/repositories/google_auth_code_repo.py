...
async def get_by_code(session: AsyncSession, code: str) -> Optional[GoogleAuthCode]:
    """Fetches an auth code record by its code string."""
    result = await session.execute(select(GoogleAuthCode).where(GoogleAuthCode.code == code))
...
async def delete_by_id(session: AsyncSession, code_id: uuid.UUID) -> None:
    ...
    await session.execute(delete(GoogleAuthCode).where(GoogleAuthCode.id == code_id))
    await session.commit()
...
async def delete_expired(session: AsyncSession) -> None:
    ...
    await session.execute(delete(GoogleAuthCode).where(GoogleAuthCode.expires_at < now))
    await session.commit()
