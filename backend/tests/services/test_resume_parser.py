...
async def sample_user(db_session):
    user_in = UserCreate(name="Parser Test User", email="parser.test@example.com", password="secret1Password")
...
async def test_pdf_parsing_success(db_session, sample_user, resume_service, temp_storage):
    ...
    upload_file = UploadFile(file=BytesIO(content), filename=file_name, headers={"content-type": "application/pdf"})
    ...
    with (
        patch("app.database.session.async_session_factory", return_value=MockSessionContext(db_session)),
        patch("app.core.parsers.fitz.open", return_value=mock_doc) as mock_fitz_open,
    ):
        resume = await resume_service.upload_resume(
            session=db_session, user_id=sample_user.id, file=upload_file, background_tasks=bg_tasks
        )
