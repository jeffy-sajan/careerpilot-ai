...
async def sample_user(db_session):
    user_in = UserCreate(name="Token User", email="token@example.com", password="secret1Password")
...
async def test_refresh_token_lifecycle(db_session, sample_user):
    ...
    token = await refresh_token_repo.create(
        db_session, user_id=sample_user.id, token_hash=token_hash, expires_at=expires_at
    )
